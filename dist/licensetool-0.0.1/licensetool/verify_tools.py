...
# 读取设备信息，base64加密后写入sys.info生成设备指纹
def sysInfo():
...
# 验证license中的设备指纹与有效期   
def sysInfoVerify(sysLicenseFile, privkeyFile=os.path.dirname(os.path.realpath(__file__)) + "\private.pem"):  
    with open(privkeyFile, "rb") as privatefile:
        p = privatefile.read()
        privkey = rsa.PrivateKey.load_pkcs1(p)
        # print(privkey)
    # print("os.path.dirname(os.path.realpath(__file__))=%s" % os.path.dirname(os.path.realpath(__file__)))      
    with open(sysLicenseFile, 'r') as f1:
        lines = f1.readlines()
